...
const CardContainer = styled.div`
  background-color: ${({ $modoOscuro }) => ($modoOscuro ? "rgba(31, 41, 55, 0.9)" : "#fff")};
...
const TotalSummaryItem = styled.div`
  ...
  box-shadow: inset 0 0 12px
    ${({ $modoOscuro }) => ($modoOscuro ? "rgba(0, 102, 0, 0.8)" : "rgba(120, 180, 50, 0.7)")};
  color: ${({ $modoOscuro }) => ($modoOscuro ? "#d3f9d8" : "#2f4f00")};
  font-weight: 700;
  text-shadow: 1px 1px 2px ${({ $modoOscuro }) => ($modoOscuro ? "#225522" : "#a6db3f")};
...
const TopUserSummaryItem = styled.div`
  ...
  box-shadow: inset 0 0 12px
    ${({ $modoOscuro }) => ($modoOscuro ? "rgba(255, 214, 77, 0.8)" : "rgba(178, 135, 4, 0.7)")};
  color: ${({ $modoOscuro }) => ($modoOscuro ? "#fffde7" : "#5a4500")};
  font-weight: 700;
  text-shadow: 1px 1px 2px ${({ $modoOscuro }) => ($modoOscuro ? "#5a4300" : "#f1d844")};
  padding: 0.5rem 0.7rem;
  border-radius: 6px;
  flex: 1 1 30%;
  min-width: 100px;
...
const TipoTransaccionSummaryItem = styled.div`
  ...
  box-shadow: inset 0 0 12px
    ${({ $modoOscuro }) => ($modoOscuro ? "rgba(0, 102, 204, 0.8)" : "rgba(82, 119, 181, 0.7)")};
  color: ${({ $modoOscuro }) => ($modoOscuro ? "#d4e9ff" : "#1c2f57")};
  font-weight: 700;
  text-shadow: 1px 1px 2px ${({ $modoOscuro }) => ($modoOscuro ? "#0b2350" : "#a8b9d6")};
  padding: 0.5rem 0.7rem;
  border-radius: 6px;
  flex: 1 1 30%;
  min-width: 100px;
...
const ToggleHeader = styled.div`
  ...
  cursor: pointer;
  padding: 0.8rem 1.4rem; /* Más alto y ancho */
  color: ${({ $modoOscuro }) => ($modoOscuro ? "#fff" : "#222")};
  display: flex;
  justify-content: center;
  align-items: center;
  gap: 0.5rem;
  border-radius: 9999px; /* Redondeado total tipo chip */
   background-color: ${({ $modoOscuro }) => ($modoOscuro ? "rgba(31, 41, 55, 0.9)" : "#ddd")};
  box-shadow: 0 2px 6px rgba(0, 0, 0, 0.15); /* Sombrado ligero */
  ...
  &:focus {
     background-color: ${({ $modoOscuro }) => ($modoOscuro ? "rgba(31, 41, 55, 0.9)" : "#ddd")};
    outline: none;
    transform: scale(1.03); /* Pequeño efecto de zoom */
  }
`;
...
const Icon = styled.span`
  ...
  font-size: 1.1rem; /* Ícono más grande */
  user-select: none;
`;
        ...
        <>
          <Summary>
            <TotalSummaryItem $modoOscuro={modoOscuro}>
              ...
              <SummaryLabel $modoOscuro={modoOscuro}>Total transacciones</SummaryLabel>
            </TotalSummaryItem>

            <TopUserSummaryItem $modoOscuro={modoOscuro}>
              <SummaryNumber>{topUsuarios[0]?.[1] || 0}</SummaryNumber>
              <SummaryLabel $modoOscuro={modoOscuro}>Transacciones del usuario top</SummaryLabel>
              <SmallText $modoOscuro={modoOscuro}>{topUsuarios[0]?.[0] || "N/A"}</SmallText>
            </TopUserSummaryItem>

            <TipoTransaccionSummaryItem $modoOscuro={modoOscuro}>
              <SummaryNumber>{dataPorTipo.length}</SummaryNumber>
              <SummaryLabel $modoOscuro={modoOscuro}>Tipos distintos de transacción</SummaryLabel>
          ...
          <CollapsibleContent open={open} $modoOscuro={modoOscuro} aria-hidden={!open}>
